...
The script generates a canvas image (window capture) to identify each frame and leech the outputted graphics. The majority of modules & core scripts use these frames in bitmap finding algorithms (pattern matching with different colorspaces) to determine when and where to perform an aciton with Pyautogui. 
